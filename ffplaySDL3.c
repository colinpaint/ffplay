  ...
  while (len > 0) {
    ...
    if (!is->muted && is->audio_buf && is->audio_volume == SDL_MIX_MAXVOLUME)
      ...
      if (!is->muted && is->audio_buf)
        SDL_MixAudioFormat (stream, (uint8_t *)is->audio_buf + is->audio_buf_index, SDL_AUDIO_S16, len1, is->audio_volume);
      }
...
//}}}
//{{{
static int videoOpen (VideoState* is) {
...
//}}}
//{{{
static void drawVideoDisplay (VideoState* is) {
...
//}}}
//{{{
static int queuePicture (VideoState* is, AVFrame* src_frame, double pts, double duration,
...
static int video_thread (void* arg) {
  ...
  for (;;) {
    ...
    while (ret >= 0) {
      ...
      ret = queuePicture (is, frame, pts, duration, fd ? fd->pkt_pos : -1, is->viddec.pkt_serial);
...
static int subtitle_thread (void* arg) {
  ...
  for (;;) {
    ...
    int got_subtitle;
    if ((got_subtitle = decodeFrame (&is->subdec, NULL, &sp->sub)) < 0)
      break;
...
int main (int argc, char** argv) {
  ...
  SDL_EventState (SDL_EVENT_SYSWM, SDL_IGNORE);
  SDL_EventState (SDL_EVENT_USER, SDL_IGNORE);
  ...
  if (!display_disable) {
    ...
    if (alwaysontop)
      ...
      if (window) {
        renderer = SDL_CreateRenderer (window, "accel", SDL_RENDERER_ACCELERATED | SDL_RENDERER_PRESENTVSYNC);

        if (!renderer) {
          av_log (NULL, AV_LOG_WARNING, "Failed to initialize a hardware accelerated renderer: %s\n", SDL_GetError());
          renderer = SDL_CreateRenderer (window, "simple", 0);
          }
        }
